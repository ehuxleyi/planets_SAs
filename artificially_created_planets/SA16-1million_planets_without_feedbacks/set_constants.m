...
nreruns = 1000000;                  % no of times each planet is rerun from scratch
nruns = nplanets*nreruns;     % total number of runs to carry out
ntaskruns = 5000;               % max number of runs per task; (parallel code)
...
str = datestr(now);
savename = sprintf('SA16_%dx%d_%s%s%s', nplanets, nreruns, ...
   str(1:2), str(4:6), str(8:11));
% savename = sprintf('SA16_1x1000000_28Mar2017');
