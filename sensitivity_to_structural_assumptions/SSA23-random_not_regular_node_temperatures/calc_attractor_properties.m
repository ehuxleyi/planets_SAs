...
for kk = 2:nnodes
    if ((feedbacks(kk) < 0) && (feedbacks(kk-1) > 0))  % found an attractor
        ...
        % point is situated
        % <<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
        thisgap = Tnodes(kk) - Tnodes(kk-1);
        attr(nattr,1) = Tnodes(kk-1) + ...
            ((thisgap*feedbacks(kk-1)) / ...
        ...
        if (nn == 1)     % reached left-hand edge
            ...
            % <<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
            thisgap = Tnodes(nn) - Tnodes(nn-1);
            min_aT = Tnodes(nn) - ...
                (feedbacks(nn) * thisgap / (feedbacks(nn)-feedbacks(nn-1)));
        end;
        attr(nattr,9) = min_aT;
        ...
        if (nn == nnodes)     % reached right-hand edge
            ...
            % <<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
            thisgap = Tnodes(nn+1) - Tnodes(nn);
            max_aT = Tnodes(nn) + ...
                ((-feedbacks(nn)) * thisgap / (feedbacks(nn+1)-feedbacks(nn)));
        end;
