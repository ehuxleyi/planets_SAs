...
for tprc = 1 : length(pl)  % the index of a planet/run in the task array
    ...
    runs(rr).nmuts = tr(tprc).nmuts;
    for kk = 1:8
        runs(rr).type(kk) = tr(tprc).type(kk);
        runs(rr).ampl(kk) = tr(tprc).ampl(kk);
        runs(rr).sign(kk) = tr(tprc).sign(kk);
        runs(rr).posc(kk) = tr(tprc).posc(kk);
        runs(rr).time(kk) = tr(tprc).time(kk);
    end;
end;