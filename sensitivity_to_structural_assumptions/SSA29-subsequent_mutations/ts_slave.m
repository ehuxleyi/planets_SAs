...
for plc = 1 : length(tplanets)    % plc = counter for planets
    ...
    % <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>>>
    tfinal	= run_events(1,1);    
    steps   = 0;
    limit   = 1000000;
    refine  = 1;   % no extra output between time steps
    % <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>>>
    eventc = 0; % event counter
    pertc = 1;  % perturbation counter
    mutc = 1;   % mutation counter
    ...
    % <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>>>
    % plot the current feedbacks
    if (verbose)
        plot_current_feedbacks;
        drawnow;
    end;
    
    %% OTHER STAGES OF RUN
    % continue the run from event to event until end reached
    % or planet gone sterile
    % <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>>>
    while (((round(t(end))) < max_duration) && (y(end,1) > Tmin) && ...
            (y(end,1) < Tmax) && (eventc < nevents))
        
        % ----- Sort out mechanics/parameterisation of simulation -----
        % <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>>>
        eventc = eventc + 1;    % increment event counter
        tstart	= t(end);     % continue from end of last stage
        % <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>>>
        if (eventc < nevents)   % if more events left to do
            tfinal	= run_events((eventc+1),1); % until next event
        else     % if all events done
        ...
        % ----- Set up initial conditions -----
        % <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>>>
        % if the event at this point is a perturbation then alter T
        if (run_events(eventc,2) == 1)
            y(end,1) = y(end,1) + perturbations(pertc,2);  % add perturbation
            pertc = pertc + 1;
            % else if the event at this point is a mutation then alter
            % dT/dt
        elseif (run_events(eventc,2) == 2)
            % add this next mutation to the ODE for dT/dt
            update_mut_effects;
            mutc = mutc + 1;
        else
