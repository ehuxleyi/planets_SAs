...
% <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>>>
% show times of mutations as blue circles whose size is proportional to the
% amplitude of the mutation
for kk = 1:nmuts
    symsize = round(abs(muts(kk,2)/3.0));  % scale size to mutation
    if (symsize < 1)
        symsize =1;
    end
    plot((muts(kk,5)/1e3), (Tmin-10), 'ob', 'MarkerSize', symsize);
    hold on;
end;
