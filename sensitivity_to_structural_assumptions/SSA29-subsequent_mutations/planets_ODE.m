...
% <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>>>
% calculate the effect of mutations, at this temperature
mut_effect_at_this_T = interp1(Tvec, mut_effect_on_dT, T);
...
elseif (node_bef == node_aft)
    ...
    % luminosity
    % <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>>>
    out(1) = temp + (trend * (time/1e6)) + mut_effect_at_this_T;
end;
