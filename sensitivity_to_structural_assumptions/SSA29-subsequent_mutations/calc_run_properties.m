...
trendslope = coeffs(1) * 1e6;         % slope in (degrees C) per By
Ttrend_observed = atan(trendslope/100) * 180.0 / pi; % units of (degrees angle)
...
% <<<<<<<<<<<<<<SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
% add mutations information to the runs array
runs(plc).nmuts = nmuts;
for kk = 1:8
    runs(plc).type(kk) = NaN;
    runs(plc).ampl(kk) = NaN;
    runs(plc).sign(kk) = NaN;
    runs(plc).posc(kk) = NaN;
    runs(plc).time(kk) = NaN;
end
for kk = 1:nmuts
    runs(plc).type(kk) = muts(kk,1);
    runs(plc).ampl(kk) = muts(kk,2);
    runs(plc).sign(kk) = muts(kk,3);
    runs(plc).posc(kk) = muts(kk,4);
    runs(plc).time(kk) = muts(kk,5);
end
