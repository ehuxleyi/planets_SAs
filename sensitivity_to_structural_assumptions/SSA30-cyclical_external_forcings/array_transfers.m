...
for tprc = 1 : length(pl)  % the index of a planet/run in the task array
    ...
    % <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>
    planets(pp).ncycles  = tp(tprc).ncycles;       % number of cycles
    for qq = 1:8
        planets(pp).cycle_periods(qq) = tp(tprc).cycle_periods(qq);        % period of cycle
        planets(pp).cycle_amplitudes(qq) = tp(tprc).cycle_amplitudes(qq);     % amplitude of cycle
    end;    
    planets(pp).max_cycle_amplitude = tp(tprc).max_cycle_amplitude;    
