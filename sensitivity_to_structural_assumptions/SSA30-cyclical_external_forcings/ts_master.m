...
for ii = 1:nplanets
    ...
    % <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>
    planets(ii).ncycles = ncycles;          % number of periodic fluctuations
    for qq = 1:8
        planets(ii).cycle_periods(qq) = cperiods(qq);        % period of cycle
        planets(ii).cycle_amplitudes(qq) = camplitudes(qq);     % amplitude of cycle
    end
...
for jj = 1 : njobs
    ...
    for tt = taskbc:taskec
        ...
        % <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>
        task_ncycles   = double(zeros([nr 1])); % numbers of cycles of each planet
        task_cperiods  = double(zeros([nr 8]));   % periods of cyclical forcings
        task_camps     = double(zeros([nr 8]));   % amplitudes of cyclical forcings
        ...
        for kk = 1 : nr   % for each run in this task
            ...
            % <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>
            task_ncycles(kk) = planets(pp).ncycles;
            for qq = 1:8
                task_cperiods(kk,qq) = planets(pp).cycle_periods(qq);
                task_camps(kk,qq) = planets(pp).cycle_amplitudes(qq);
            end
        ...
        s = struct('arg1', savename, ...   % string
            ...
            'arg8', task_ncycles, ...    % array of doubles
            'arg9', task_cperiods, ...    % array of doubles
            'arg10', task_camps, ...    % array of doubles
            'arg11', task_trends, ...       % vector of doubles
            'arg12', task_blambdas, ...     % vector of doubles
            'arg13', task_mlambdas, ...    % vector of doubles
            'arg14', task_llambdas);        % vector of doubles
