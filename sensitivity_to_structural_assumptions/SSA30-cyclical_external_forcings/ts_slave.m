...
% <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>
%   pnumcycs   = list of the numbers of cycles of each planet
%   pcperiods  = lists of the cycle periods for each planet
%   pcamps     = lists of the cycle amplitudes for each planet
...
% <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>
pnumcycs = argss.arg8;
pcperiods = argss.arg9;
pcamps = argss.arg10;
ptrends = argss.arg11;
pblambdas = argss.arg12;
pmlambdas = argss.arg13;
pllambdas = argss.arg14;
...
for plc = 1 : length(tplanets)    % plc = counter for planets
    ...
    % <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>
    % get this planet's cyclical forcings
    ncycles = pnumcycs(plc);               % number of cycles
    for qq = 1:8
        cperiods(qq) = pcperiods(plc,qq);
        camplitudes(qq) = pcamps(plc,qq);
    end;
    ...
    for qq = 1:8
        planets(plc).cycle_periods(qq) = cperiods(qq);
        planets(plc).cycle_amplitudes(qq) = camplitudes(qq);
    end;
