...
for plc = 1 : length(tplanets)    % plc = counter for planets
    ...
    y0 = [ Tinit H1init H2init H3init 0.0 ];
    ...
    % <<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
    while (((round(t(end))) < max_duration) && ...
            (y(end,1) > Tmin)  && (y(end,1) < Tmax) && ...
            (y(end,2) > H1min) && (y(end,2) < H1max) && ...
            (y(end,3) > H2min) && (y(end,3) < H2max) && ...
            (y(end,4) > H3min) && (y(end,4) < H3max) && ...
            (stcounter < pcounter))
