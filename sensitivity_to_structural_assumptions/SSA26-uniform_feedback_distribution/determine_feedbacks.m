...
for nn = 1:nnodes
    ...
    % <<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>
    % r2 is uniform between 0 and 1. Change to uniform between -1 and 1
    r2v2 = (2.0 * r2(ii,nn)) - 1.0;
    % to maintain the same average absolute magnitude of feedback, but with
    % a uniform rather than normal distribution
    Tfeedbacks(nn) = r2v2 * fsd * 1.5;
end;
