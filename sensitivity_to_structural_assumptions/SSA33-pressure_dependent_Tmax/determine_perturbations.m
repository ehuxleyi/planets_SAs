...
% <<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>
% perturb P, not just T
% (:,1) = time
% (:,2) = perturbation to T
% (:,3) = perturbation to P
perturbations = double(zeros([pcounter 3]));
...
for ppp = 1:bpcounter
    ...
    % increase the magnitude by a factor of 2 for P because its range is
    % larger
    magnitudeP = normrnd(2*bigp_mean, 2*bigp_std);     % degrees C
    signT = round(rand)*2 - 1;  % randomly an increase or a decrease
    signP = round(rand)*2 - 1;  % randomly an increase or a decrease
    perturbations(ppp, 1) = ptime;              % store time...
    perturbations(ppp, 2) = magnitudeT * signT;    % ...and impact
    perturbations(ppp, 3) = magnitudeP * signP;    % ...and impact    
...
for ppp = (bpcounter+1):(bpcounter+mpcounter)
    ...
    % increase the magnitude by a factor of 2 for P because its range is
    % larger
    magnitudeP = normrnd(2*midp_mean, 2*midp_std);     % degrees C
    signT = round(rand)*2 - 1;  % randomly an increase or a decrease
    signP = round(rand)*2 - 1;  % randomly an increase or a decrease
    perturbations(ppp, 1) = ptime;              % store time...
    perturbations(ppp, 2) = magnitudeT * signT;    % ...and impact
...
for ppp = (bpcounter+mpcounter+1):pcounter
    ...
    % increase the magnitude by a factor of 2 for P because its range is
    % larger
    magnitudeP = normrnd(2*littlep_mean, 2*littlep_std);     % degrees C
    signT = round(rand)*2 - 1;  % randomly an increase or a decrease
    signP = round(rand)*2 - 1;  % randomly an increase or a decrease
    perturbations(ppp, 1) = ptime;              % store time...
    perturbations(ppp, 2) = magnitudeT * signT;    % ...and impact
