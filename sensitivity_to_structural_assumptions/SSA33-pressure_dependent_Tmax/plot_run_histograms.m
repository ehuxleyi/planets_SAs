...
% run
subplot(6,1,6);
x = ((Tmin-10)+((Trange0+20)/(2*nbinsd))) : ((Trange0+20)/nbinsd) : ...
    ((Tmax0+10)-((Trange0+20)/(2*nbinsd)));
bar(x,fr_modeTs(:,1));
xlim([(Tmin-10) (Tmax0+10)]);
...
edges = (Tmin-10.0) : bargap : (Tmax0+10.0);
hcounts = histc(allruns_modeTs,edges);
hcounts(end-1) = hcounts(end-1) + hcounts(end);
bar(edges,hcounts,'histc');
xlim([(Tmin-10.0) (Tmax0+10.0)]);
...
x = ((Tmin-10)+(thisgap/2.0)) : thisgap : ((Tmax0+10)-(thisgap/2.0));
...
xlim([(Tmin-10) (Tmax0+10)]); ylim([0 4]);
set(gca,'Ytick',0:4,'YtickLabel',{'0','1','2','3','4'});
