...
for rr=1:nruns
    ...
    % <<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>
    allruns_nodes(rr) = planets(pp).nTnodes;
    ...
    % <<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>
    initTbin = 1+floor(nbinsd*(runs(rr).Tinit-Tmin)/Trange0);
    ...
    % <<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>
    modeTbin = 1+floor(nbinsd*(runs(rr).Tmode-(Tmin-10.0))/(Trange0+20.0));
