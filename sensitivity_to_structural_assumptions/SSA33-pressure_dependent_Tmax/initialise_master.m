...
% <<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
Pnodes = double(zeros([1 nnodes_max]));    % P at each node
Pfeedbacks = double(zeros([1 nnodes_max]));% feedback (dP/dt) at each node
...
for pp = 1:nplanets
    ...
    % <<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
    for qq = 1:nnodes_max
        planets(pp).Pnodes(qq) = NaN;         % P of node
        planets(pp).Pfeedbacks(qq) = NaN;     % dP/dt of node
    end
end
...
% <<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
r1 = rand([nplanets 4]);  % 4 instead of 1 because now 4 ODEs
r2 = randn([nplanets nnodes_max 4]);  % 4 instead of 1 because now 4 ODEs
r3 = randn([nplanets 4]);  % 4 instead of 1 because now 4 ODEs
