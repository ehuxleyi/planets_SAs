...
for jj = 1 : njobs
    ...
    % <<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
    task_nnodes     = double(zeros([nr 2])); % numbers of nodes of each planet for each ODE
    task_nodes      = double(zeros([nr nnodes_max 2])); % value at each node of each planet for each ODE
    task_feedbacks  = double(zeros([nr nnodes_max 2])); % feedback at each node of each planet for each ODE
    task_trends     = double(zeros([nr 2])); % trend for each planet for each ODE
    ...
    task_ks         = double(zeros([nr 2])); % trend for each planet for each ODE
    ...
    for kk = 1 : nr   % for each run in this task
        ...
        task_nnodes(kk,2) = planets(pp).nPnodes;
        ...
        for qq = 1:task_nnodes(kk,2)
            task_nodes(kk,qq,2)     = planets(pp).Pnodes(qq);
            task_feedbacks(kk,qq,2) = planets(pp).Pfeedbacks(qq);
        end
        ...
        task_trends(kk,2) = planets(pp).trendP;
