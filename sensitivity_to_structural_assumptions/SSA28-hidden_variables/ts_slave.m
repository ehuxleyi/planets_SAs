...
for plc = 1 : length(tplanets)    % plc = counter for planets
    ...
    nH1nodes = pnumnodes(plc,2);               % number of nodes
    nH2nodes = pnumnodes(plc,3);               % number of nodes
    nH3nodes = pnumnodes(plc,4);               % number of nodes
    Tgap = Trange / (nTnodes-1);
    H1gap = H1range / (nH1nodes-1);
    H2gap = H2range / (nH2nodes-1);
    H3gap = H3range / (nH3nodes-1);
    for qq = 1:nnodes_max
    ...
    % <<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
    for qq = 1:nnodes_max
        H1nodes(qq) = pnodes(plc,qq,2);             % T of nodes
        H1feedbacks(qq) = pfeedbacks(plc,qq,2);     % dT/dt of nodes
    end;
    for qq = 1:nnodes_max
        H2nodes(qq) = pnodes(plc,qq,3);             % T of nodes
        H2feedbacks(qq) = pfeedbacks(plc,qq,3);     % dT/dt of nodes
    end;    
    for qq = 1:nnodes_max
        H3nodes(qq) = pnodes(plc,qq,4);             % T of nodes
        H3feedbacks(qq) = pfeedbacks(plc,qq,4);     % dT/dt of nodes
    end;
    ...
    trendH1 = ptrends(plc,2);       % heating or cooling trend
    trendH2 = ptrends(plc,3);       % heating or cooling trend
    trendH3 = ptrends(plc,4);       % heating or cooling trend
    ...
    [Tinit, H1init, H2init, H3init] = ...
        determine_initial_values(Tmin, Trange);
    ...
    while (((round(t(end))) < max_duration) && (y(end,1) > Tmin) && ...
        ...
        y(end,2) = y(end,2) + perturbations(stcounter,3);  % add perturbation
        y(end,3) = y(end,3) + perturbations(stcounter,4);  % add perturbation
        y(end,4) = y(end,4) + perturbations(stcounter,5);  % add perturbation
        y0 = y(end,:);
    ...
    planets(plc).nH1nodes  = nH1nodes;           % number of nodes
    planets(plc).nH2nodes  = nH2nodes;           % number of nodes
    planets(plc).nH3nodes  = nH3nodes;           % number of nodes
    for qq = 1:nnodes_max
        planets(plc).Tnodes(qq) = Tnodes(qq); % T of each node
        planets(plc).Tfeedbacks(qq) = Tfeedbacks(qq);  % dT/dt of each node
    end;
    for qq = 1:nnodes_max
        planets(plc).H1nodes(qq) = H1nodes(qq); % H1 of each node
        planets(plc).H1feedbacks(qq) = H1feedbacks(qq);  % dH1/dt of each node
    end;
        for qq = 1:nnodes_max
        planets(plc).H2nodes(qq) = H2nodes(qq); % H2 of each node
        planets(plc).H2feedbacks(qq) = H2feedbacks(qq);  % dH2/dt of each node
    end;
        for qq = 1:nnodes_max
        planets(plc).H3nodes(qq) = H3nodes(qq); % H3 of each node
        planets(plc).H3feedbacks(qq) = H3feedbacks(qq);  % dH3/dt of each node
    end;
    % <<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
    planets(plc).trendT  = trendT;              % heating or cooling trend
    planets(plc).trendH1 = trendH1;              % heating or cooling trend
    planets(plc).trendH2 = trendH2;              % heating or cooling trend
    planets(plc).trendH3 = trendH3;              % heating or cooling trend
    ...
    % <<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
    planets(plc).k1 = k1;              % coefficient
    planets(plc).k2 = k2;              % coefficient
    planets(plc).k3 = k3;              % coefficient
    planets(plc).k4 = k4;              % coefficient
    planets(plc).k5 = k5;              % coefficient
    planets(plc).k6 = k6;              % coefficient
