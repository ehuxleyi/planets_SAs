...
for ii = 1:nplanets
    ...
    % <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
    % [4] Randomly initialise the planet's surface temperature which will
    % be the same for each rerun of this planet
    Tinit = determine_initial_T(Tmin, Trange);
    
    % save the key planet details in an array
    % allocate this planet's set of intrinisic feedbacks
    ...
    % <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
    planets(ii).initT = Tinit;            % starting temperature
...
for jj = 1 : njobs
    ...
    for tt = taskbc:taskec
        ...
        % <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
        task_initTs    = double(zeros([nr 1])); % starting T for each planet
        ...
        for kk = 1 : nr   % for each run in this task
            ...
            % <<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>>
            task_initTs(kk) = planets(pp).initT;
        end;
        ...
        s = struct('arg1', savename, ...   % string
            ...
            'arg12', task_initTs);         % vector of doubles
