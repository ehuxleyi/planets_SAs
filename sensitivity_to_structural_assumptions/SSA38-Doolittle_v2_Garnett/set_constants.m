...
ntaskruns = 250;             % max number of runs per task; (optimised parallel code) (= a bit less than 2 hours per task)
...
% <<<<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>
nmutations_max  = 15;	   % maximum number of mutations for this planet
nmutation_size_max = nnodes_max;	   % maximum number of nodes a mutation can affect
mutation_sd = fsd;		   % std. dev of the amplitude of mutations

% automatically construct a results directory name from the date, size and
% a stub indicating the platform on which it is run
str = datestr(now);
savename = sprintf('SA38_I4_%dx%d_%s%s%s', nplanets, nreruns, ...
    str(1:2), str(4:6), str(8:11));
% savename = sprintf('SA38_I4_1000x50_19May2017');
