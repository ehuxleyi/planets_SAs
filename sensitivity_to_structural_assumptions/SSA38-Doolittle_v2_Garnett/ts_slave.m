...
for plc = 1 : length(tplanets)    % plc = counter for planets
    ...
    % <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>
    mutcounter = 0;
    % whether the next event is a perturbation (0) or a mutation (1)
    is_next_mut = perturbations(1,1) > mutation_times(1);
    ...
    while (((round(t(end))) < max_duration) && (y(end,1) > Tmin) && ...
        ...
        % <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>
        %check whether the next event is a perturbation or a mutation
        if (is_next_mut == 0) % perturbation
        ...
        % <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>
            ...
            % <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>
            mutcounter = mutcounter + 1; %increment mutation counter
            tstart = t(end);
            
            % <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>
            % implement mutation (add to feedbacks)
            Tfeedbacks = Tfeedbacks + mutations(:, mutcounter)'; % add our mutation
            
            y0 = y(end,:);    
        end
        
        % <<<<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>>>
        %find if we have a mutation or a perturbation next
        %if we're out of mutations
        if ((mutcounter == nmutations_max) || ...
                (isnan(mutation_times(mutcounter+1))))
            is_next_mut = 0;
        else
            %find what's next
            is_next_mut = perturbations((stcounter+1),1) > ...
                mutation_times(mutcounter+1);
        end
        if(is_next_mut == 1) %mutation
            tfinal = mutation_times(mutcounter+1);
        else %perturbation
            %if we're also out of perturbations, go until the end
            if(stcounter >= pcounter)
                tfinal	= max_duration;
            else
                tfinal	= perturbations((stcounter+1),1); % until next perturbation
                while(tfinal <= tstart)
                    tfinal = tfinal + 0.1; %shouldnever fire
                end
            end
        end
