...
runs(plc).nmutations = nmutations;
for kk = 1:nmutations_max
    runs(plc).msize(kk) = NaN;
    runs(plc).time(kk) = NaN;
end
for kk = 1:nmutations_max
    runs(plc).msize(kk) = msizes(kk);  % largest out of all the node impacts
    runs(plc).time(kk) = mutation_times(kk);
end
