...
% <<<<<<<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>>>>>
% generate a random non-linearity to apply to the forcing
sign = round(rand)*2 - 1;

% generate a degree of deviation from a straight line (where a straight
% line is the default case, i.e. where the trend increases in strength as a
% linear function of time)
deviation = 1.0 + 5.0*rand;

if (sign > 0)
    % increasing as a power of time
    gamma = deviation;      % randomly between 1 and 6
else
    % increasing as a power of 1/time
    gamma = 1.0/deviation;  % randomly between 1/6 and 1
end;
