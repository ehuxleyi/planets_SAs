...
% <<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>
Tmin0 = double(-20.0);        % minimum habitable temperature (lower limit of habitable range) (degrees C)
Tmax0 = double(70.0);         % maximum habitable temperature (upper limit of habitable range) (degrees C)
Trange0 = Tmax0-Tmin0;        % width of habitable envelope (degrees C)
...
str = datestr(now);
savename = sprintf('SA21_I4_%dx%d_%s%s%s', nplanets, nreruns, ...
   str(1:2), str(4:6), str(8:11));
% savename = sprintf('SA21_I4_1000x50_10Apr2017');
