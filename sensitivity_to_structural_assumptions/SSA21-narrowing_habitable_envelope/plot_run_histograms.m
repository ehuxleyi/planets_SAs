...
% <<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>
% <<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>
% ~25 different lines changed in this file, so they are not all
% individually noted
...
% run
subplot(6,1,6);
x = ((Tmin0-10)+((Trange0+20)/(2*nbinsd))) : ((Trange0+20)/nbinsd) : ...
    ((Tmax0+10)-((Trange0+20)/(2*nbinsd)));
bar(x,fr_modeTs(:,1));
xlim([(Tmin0-10) (Tmax0+10)]);
...
% run
subplot(6,1,6);
bargap = (Trange0+20.0)/nbinsd;
edges = (Tmin0-10.0) : bargap : (Tmax0+10.0);
hcounts = histc(allruns_modeTs,edges);
hcounts(end-1) = hcounts(end-1) + hcounts(end);
bar(edges,hcounts,'histc');
xlim([(Tmin0-10.0) (Tmax0+10.0)]);
...
subplot(6,1,6);
thisgap = (Trange0+20.0)/nbinsd;
x = ((Tmin0-10)+(thisgap/2.0)) : thisgap : ((Tmax0+10)-(thisgap/2.0));
...
xlim([(Tmin0-10) (Tmax0+10)]); ylim([0 4]);
set(gca,'Ytick',0:4,'YtickLabel',{'0','1','2','3','4'});
