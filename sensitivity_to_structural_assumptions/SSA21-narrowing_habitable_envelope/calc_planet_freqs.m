...
for pp=1:nplanets
    ...
    if (isfinite(planets(pp).max_attr_pos1))
        % <<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>
        pos1bin = 1+floor(nbinsd*(planets(pp).max_attr_pos1-Tmin0)/Trange0);
        if (pos1bin == (nbinsd+1))
            pos1bin = nbinsd;
        end;
    else
    ...
    if (isfinite(planets(pp).max_attr_dist1))
        % <<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>
        dist1bin = 1+floor(nbinsd*(planets(pp).max_attr_pos1-Tmin0)/Trange0);
    ...
    if (isfinite(planets(pp).max_attr_pos2))
        % <<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>
        pos2bin = 1+floor(nbinsd*(planets(pp).max_attr_pos2-Tmin0)/Trange0);
        if (pos2bin == (nbinsd+1))
            pos2bin = nbinsd;
        end;
    else
