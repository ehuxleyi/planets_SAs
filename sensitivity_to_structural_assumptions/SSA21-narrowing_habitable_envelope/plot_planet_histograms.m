...
% <<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>
% <<<<<<<< SPECIAL FOR THIS SA >>>>>>>>>
% ~100 different lines changed in this file, so they are not all
% individually noted
...
subplot(6,1,3);
x = (Trange0/(2*nbinsd)) : (Trange0/nbinsd) : ...
    (((2*nbinsd)-1)*Trange0/(2*nbinsd));
bar(x,fr_widths(:,1));
xlim([0 Trange0]);
...
% (first definition)
subplot(6,1,3);
x = (Tmin0+(Trange0/(2*nbinsd))) : (Trange0/nbinsd) : ...
    (Tmax0-(Trange0/(2*nbinsd)));
bar(x,fr_pos1s(:,1));
xlim([Tmin0 Tmax0]);
...
subplot(6,1,4);
x = (Trange0/(4*nbinsd)) : (Trange0/(2*nbinsd)) : ...
    (((2*nbinsd)-1)*Trange0/(4*nbinsd));
bar(x,fr_dist1s(:,1));
xlim([0.0 Trange0*0.55]);
...
subplot(6,1,3);
bargap = Trange0/nbinsd;
edges = 0.0:bargap:Trange0;
...
subplot(6,1,4);
bargap = (Trange0/2.0)/nbinsd;
edges = 0.0:bargap:(Trange0/2.0);
...
subplot(6,1,3);
thisgap = Trange0/nbinsd;
x = (thisgap/2.0) : thisgap : (Trange0-(thisgap/2.0));
plot(x,ratio_widths(:),'-bo');
xlim([0 Trange0]); ylim([0 4]);
set(gca,'Ytick',0:4,'YtickLabel',{'0','1','2','3','4'});
...
subplot(7,1,3);
thisgap = Trange0/nbinsd;
x = (Tmin0+(thisgap/2.0)) : thisgap : (Tmax0-(thisgap/2.0));
plot(x,ratio_pos1s(:),'-bo');
xlim([Tmin0 Tmax0]); ylim([0 4]);
set(gca,'Ytick',0:4,'YtickLabel',{'0','1','2','3','4'});
...
% from midpoint (first definition)
subplot(7,1,4);
thisgap = Trange0/(2*nbinsd);
x = (thisgap/2.0) : thisgap : ((Trange0/2.0)-(thisgap/2.0));
plot(x,ratio_dist1s(:),'-bo');
xlim([0.0 Trange0*0.55]); ylim([0 4]);
set(gca,'Ytick',0:4,'YtickLabel',{'0','1','2','3','4'});
