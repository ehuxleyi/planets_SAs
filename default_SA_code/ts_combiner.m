% script to combine the results of several simulations into one results
% file

set_constants;
verbose = 0;        % '1' = lots of print statements and plots, '0' = few
% don't use some of the values set in set_constants
...
% first check through the directories to combine, in order to check that
% everything is there and in order. Also in order to find out how many
% planets and runs there are in total, in order to know the size of the
% arrays to set up
...
% check that same number of reruns in all
...
% now go through a second time, reading in the results from each of the
% directories
...
for ii = 1:length(places)
    ...
    for tt = 1 : nt    % for each task
        ...
        for tpc = 1 : length(pl)  % this is the index of a planet in the task array
            ...
            % now calculate the index of the planet in the total combined
            % array
            if (ii == 1)
                pp = tp(tpc).pnumber;
            else
        ...
        for tpr = 1 : (length(rl)*length(pl))  % this is the index of a run in the task array
...
% save the combined results to the new directory, also plot them
savename = dest;
