2,1)% script to set the values of constants used in the simulation
%--------------------------------------------------------------------------
...
nplanets = 1;             % no. of distinct planets
nreruns = 1000;                 % no of times each planet is rerun from scratch
nruns = nplanets*nreruns;     % total number of runs to carry out
ntaskruns = 5;             % max number of runs per task; (optimised parallel code) (= a bit less than 2 hours per task)
...
str = datestr(now);
savename = sprintf('XX_%dx%d_%s%s%s', nplanets, nreruns, ...
    str(1:2), str(4:6), str(8:11));
% savename = sprintf('XX_10x10_14Dec2016');
