...
% #####  STEM PLOTS OF PERCENTAGE SUCCESS

f = figure(74);
set (f, 'Position', [100 200 800 600]);

% specify the colour to be used for the simulation results
...
% ##### STANDARD PLOTS OF PERCENTAGE SUCCESS RATES

f = figure(75);
set (f, 'Position', [200 200 900 600]);

% standard plot of all of the sucessful planets (ones which stayed habitable
% throughout the duration of at least one of their reruns), with the height
...
fill([x1(equivindex) x1(equivindex) x1(end)+1 x1(end)+1], [0 100 100 0], ...
    [1.0 0.7 0.7], 'LineWidth', 0.1);
p2 = plot([1 x1(equivindex) x1(equivindex) x1(end)], [0 0 100 100], '-r', ...
    'LineWidth', 1.0);
% add black line and grey fill to show expected distribution for pure chance
hold on;
...
% standard plot of the top 100 most sucessful planets, with the height of
% each point being the success rate expressed as a percentage
subplot(3,1,3);
hold on;
startindex = length(A) - 99;  % -99 not -100 else get top 101 planets
if (startindex < 1)
