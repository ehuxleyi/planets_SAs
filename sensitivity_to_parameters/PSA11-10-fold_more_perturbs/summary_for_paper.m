...
fprintf('\n  SUMMARY OF RESULTS FROM ALL 500,000 (10,000 x 50) RUNS\n\n');
...
%fprintf('\n  SUMMARY OF RESULTS FROM SENSITIVITY ANALYSIS NUMBER 01\n\n');
fprintf('\n  SUMMARY OF RESULTS FROM THE FIRST 50,000 (1,000 x 50) RUNS\n\n');

% planets
npl = length(planets) / 10;
fprintf('Total number of planets for this second analysis is %d\n', npl);
c1 = 0; c2 = 0;
for ii = 1:npl
    if (planets(ii).all_survived)
        c1 = c1 + 1;
...
fprintf('Number of successful planets (some success) was %d out of %d\n', ...
    c2, npl);
fprintf('Number of perfect planets (100%% success) was %d out of %d\n\n', ...
    c1, npl);

% runs
nruns = length(runs) / 10;
fprintf('Total number of runs for this second analysis is %d\n', nruns);
c3 = 0;
for ii = 1:nruns
    if (runs(ii).result == 1)
        c3 = c3 + 1;
...
fprintf('Number of successful runs was %d out of %d (= average of %d reruns per planet)\n\n', ...
    c3, nruns, round(c3/npl));
...
% finally, do a runstest and report the result
[h,p,stats] = runstest(allruns_durations(1:nruns));
if (h == 1)
    fprintf('runstest: run durations are NOT randomly distributed (H2 is rejected, p = %8.2e)\n\n', p);
else
    fprintf('\n******* UNEXPECTED RESULT *******\n');
